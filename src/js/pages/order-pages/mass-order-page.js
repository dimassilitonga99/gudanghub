/* ═══════════════════════════════════════════════════════════════════════
   MASS ORDER PAGE — dengan Preview (tanpa tab manual)
   Manual sekarang ada di halaman Katalog
   ═══════════════════════════════════════════════════════════════════════ */
...
export function renderMassOrderPage(state) {
  return `
    ...
    <section class="format-box">
      ...
      <div class="format-note" style="margin-top: 6px; color: var(--warning);">
        <span data-icon="edit" data-icon-size="12"></span>
        Untuk barang baru (belum di katalog), gunakan tab <b>Katalog → Barang Manual</b>.
      </div>
    </section>
...
function updateMassSummary(state) {
  ...
  if (submitBtn) {
    submitBtn.innerHTML = validItems.length
      ? `${icon('file', { size: 18 })} Preview Form Order (${validItems.length} item)`
...
function showPreviewBeforeSubmit(state) {
  ...
  showPreOrderDialog({
    ...
    onConfirm: async function (config) {
      await submitMassOrder(state, validItems, config);
    },
  });
}
